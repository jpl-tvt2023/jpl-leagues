CREATE TABLE `auction_bids` (
	...
	FOREIGN KEY (`nominator_team_id`) REFERENCES `teams`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`current_high_bidder_id`) REFERENCES `teams`(`id`) ON UPDATE no action ON DELETE no action
);
...
CREATE TABLE `leagues` (
	...
	`initial_budget` integer DEFAULT 100000000 NOT NULL,
	`created_at` integer NOT NULL
);
--> statement-breakpoint
...
CREATE TABLE `trade_proposals` (
	...
	`updated_at` integer NOT NULL,
	FOREIGN KEY (`league_id`) REFERENCES `leagues`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`proposer_team_id`) REFERENCES `teams`(`id`) ON UPDATE no action ON DELETE no action,
