...
interface BracketData {
  challenger?: { qf?: TieDisplay[]; sf?: TieDisplay[]; final?: TieDisplay[] };
}
...
export default function EuropaPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((err) => {
        console.error("Error fetching Europa data:", err);
        setError("Failed to load Europa data");
      })
  ...
  // Extract UEL ties (QF, SF, Final)
  const uelQF = bracket.challenger?.qf || [];
  const uelSF = bracket.challenger?.sf || [];
  const uelFinal = bracket.challenger?.final || [];
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-purple-900 to-slate-900">
      ...
      <nav className="sticky top-0 z-50 flex flex-wrap items-center justify-between gap-2 px-4 py-3 sm:px-6 sm:py-4 lg:px-12 border-b border-white/10 bg-slate-900/80 backdrop-blur">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-sm sm:text-base">
          ...
          <Link href={`/${leagueSlug}/ucl`} className="text-gray-300 hover:text-white transition">
            UCL
          </Link>
          <Link href={`/${leagueSlug}/europa`} className="text-yellow-400 font-semibold transition">
            Europa
          </Link>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8 sm:py-12">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">UEFA Europa League</h1>
                ...
                <div key={groupLetter} className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
                  ...
                  <div className="space-y-2">
                    {group.standings.map((team, idx) => {
                      const isEuropa = idx >= 2 && idx < 4;
                      const bgClass = isEuropa
                      ...
                      return (
                        <div key={team.teamId} className={`p-3 rounded ${bgClass} flex items-center justify-between text-sm`}>
                          ...
                          <div className="flex items-center gap-4 text-gray-300 text-xs">
                            ...
                            {isEuropa && <span className="text-amber-400 font-semibold text-[10px]">UEL</span>}
                          </div>
                        </div>
                      );
                    })}
                  </div>
                  <p className="text-xs text-amber-400 mt-3">✓ Positions 3-4 qualify for Europa knockouts</p>
                </div>
              );
            })}
          </div>
        ...
        {/* UEL Knockout Bracket */}
        {(uelQF.length > 0 || uelSF.length > 0 || uelFinal.length > 0) && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {uelFinal.map((tie) => (
                    <div
                      key={tie.tieId}
                      className="rounded-xl border border-orange-500/30 bg-orange-500/5 p-4 backdrop-blur"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex-1 text-left">
                          <div className="font-semibold text-white">{tie.home?.teamName || "TBD"}</div>
                        ...
                        <div className="flex items-center gap-1 px-2 text-sm">
                          {tie.home?.score !== undefined ? (
                            <>
                              <span className="font-bold text-orange-400">{tie.home.score}</span>
                            </>
                          ) : (
                            <span className="text-gray-500">-</span>
                          )}
                      ...
                      {tie.winnerId && (
                        <div className="text-xs text-orange-400 font-semibold mt-2">🏆 EUROPA LEAGUE CHAMPION</div>
                      )}
                    </div>
                  ))}
                </div>
