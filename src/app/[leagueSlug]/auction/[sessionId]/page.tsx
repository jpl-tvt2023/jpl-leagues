...
export default function AuctionRoomPage() {
  ...
  // Initial data load — resolves the specific session named in the route (any status), not "whatever
  // is currently active." That's what makes any session (pending/active/paused/completed) enterable.
  const loadInitial = useCallback(async () => {
    ...
    try {
      ...
      const [sessRes, standingsRes, ownedRes, economyRes, squadRes, clubTiersRes] = await Promise.all([
      ...
      if (!sessRes.ok) throw new Error("Failed to load auction sessions");
      const sessJson = await sessRes.json();
      const target = (sessJson.sessions ?? []).find((s: AuctionSession) => s.id === routeSessionId);
      if (!target) throw new Error("Session not found");
      setSession(target);
      // Live fields (currentBid/nominationDeadline/intermissionUntil/allClubsAllocated/myOwnsClub) only
      // ever come back on `activeSession` — use them when this session happens to be it; otherwise the
      // SSE connect (for active/paused) or the completed-session defaults below cover the rest.
      const active = sessJson.activeSession;
      if (active && active.id === routeSessionId) {
      ...
      // Load historical bid feed (persists across page reloads/completed sessions; includes
      // missed-nomination penalties). Always fetched for the target session, regardless of status.
  ...
  // The heavier feed rebuild runs only every other tick (~6s) to bound cost. Only runs while the
  // session is actually live — nothing to poll for a pending or completed session.
  useEffect(() => {
    if (!leagueId || !sessionId || sessionStatus === "pending" || sessionStatus === "completed") return;
    let n = 0;
    const tick = () => {
      n += 1;
  ...
  }, [leagueId, sessionId, sessionStatus, resyncAuctionScreen]);
  ...
  const handleBulkAddToWishlist = async () => {
    ...
    try {
      ...
      if (ok) setUnsoldSelections(new Set());
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#38003c] via-[#1a0021] to-[#0d001a]">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-10">
        <div className="mb-4">
          <Link href={`/${leagueSlug}/auction`} className="text-sm text-yellow-400 hover:underline">← All Sessions</Link>
        </div>
        {isLoading ? (
          ...
          <div className="text-center py-16 rounded-2xl border border-white/10 bg-white/5">
            ...
            <p className="text-gray-400">This auction session doesn&apos;t exist or you don&apos;t have access to it.</p>
          </div>
        ) : (
          <>
            {/* Header bar */}
            ...
            {session.status === "pending" && (
              <div className="mb-4 rounded-2xl border border-blue-500/30 bg-gradient-to-br from-blue-500/10 to-purple-500/10 p-6 text-center backdrop-blur">
                ...
                <div className="mt-4 flex flex-wrap justify-center gap-x-5 gap-y-1 text-xs text-gray-400">
                  <span>Bid timer: {session.bidTimerSeconds ?? 20}s</span>
                  <span>Nomination timeout: {session.nominationTimeoutSeconds ?? 60}s</span>
                  <span>Intermission: {session.intermissionSeconds ?? 5}s</span>
            ...
            {session.status === "completed" && (
              <div className="mb-4 rounded-2xl border border-purple-500/30 bg-purple-500/10 p-4 text-center backdrop-blur">
                <span className="text-purple-200 text-sm font-semibold">🏁 This session has ended — showing final results below.</span>
              </div>
            )}
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {session.snakeOrder.map((tid, idx) => {
                          ...
                          const isCurrent = idx === session.currentNominatorIndex && session.status !== "completed" && session.status !== "pending";
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-5 gap-4">
              {/* Wishlist / Unsold tabs (2 cols) — FPL-player concepts only, and only relevant while
                  a player-auction session is actually live. */}
              {showWishlistPanel && (
              ...
              {/* Live Feed — expandable sold/unsold entries. Takes the full row whenever the
                  wishlist/unsold panel is hidden (club auctions, and pending/completed sessions). */}
              <div className={`${showWishlistPanel ? "lg:col-span-3" : "lg:col-span-5"} rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur`}>
